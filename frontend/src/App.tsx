    ...
    <div className="bg-gray-900 mx-auto flex items-center gap-8 px-4 sm:px-6 lg: px-8 w-full">
      ...
      <div className="flex flex-1 items-center justify-end md:justify-between">
        <nav className="p-4 bg-gray-900 text-white flex gap-4 ">
          ...
          {isLogged ? (
            <>
              <Link to="/simulation" className="hover:text-white/75">Simulación</Link>
            </>
          ) : null}
        </nav>
            ...
            {isLogged ? (
              <>
                <div className="relative group w-fit">
                  <Link to="/profile" className="flex items-center gap-2 rounded-md border border-gray-700 bg-gray-800 px-5 py-2.5 text-sm font-medium text-teal-400 transition hover:bg-gray-700 hover:text-teal-300">
                ...
                <button onClick={handleLogout} className="hidden rounded-md border border-red-900/50 px-5 py-2.5 text-sm font-medium text-red-400 transition sm:block hover:bg-red-900/20 hover:text-red-300">
                  Cerrar Sesión
                </button>
              </>
            ) : (
